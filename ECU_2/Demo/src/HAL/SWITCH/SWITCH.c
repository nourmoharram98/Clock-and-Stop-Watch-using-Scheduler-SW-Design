...
void HAL_SWITCH_Init(void)
{
	...
	for(u8 index=0;index<Number_Of_Switches;index++)
	{
		...
		Switch.Pin=arrOfSwitches[index].Switch_Pin;
		Switch.Mode=arrOfSwitches[index].SWITCH_PIN_MODE;
		GPIO_Init(&Switch);
	}
}
...
void Switch_Debouncing_Runnable(void)
{
	...
	for(u8 index=0;index<Number_Of_Switches;index++)
	{
		GPIO_GetPinValue(arrOfSwitches[index].port,arrOfSwitches[index].Switch_Pin,&current_status);
